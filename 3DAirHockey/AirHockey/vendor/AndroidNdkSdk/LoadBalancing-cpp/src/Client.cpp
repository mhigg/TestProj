...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		/**
		   @return the list of all visible game rooms.
		   @remarks
		   ...
		   @remarks
		   To show up in the lobby the IsVisible flag of a game room needs to be set to true (which is the default value).
		...
		/**
		   @return the list of the names of all visible game rooms.
...
#	pragma warning(pop)
		...
		bool Client::opCreateRoom(const JString& gameID, const RoomOptions& options, const JVector<JString>& expectedUsers)
		{
			...
			if(!options.getLobbyName().length() && options.getLobbyType() != LobbyType::DEFAULT)
			{
				EGLOG(DebugLevel::ERRORS, L"The name of a lobby with a non-default type cannot be an empty string.");
				return false;
			}
		...
		/**
		   ...
		   currently resides in will implicitly be left when entering a game room.
		   @remarks
		   If no rooms are fitting or available (all full, closed or not visible), then this operation will create a new room with the specified gameID and the specified room options.
		   @param gameID A unique identifier for the game room to create. This parameter only gets considered if no existing room matches the specified filters and a new room gets created. If this is an empty string, then the server will create
		   a room and assign a GUID as name.
		   @param options An instance of RoomOptions, that can be used to specify various options for room creation. These options only get considered if no existing room matches the filters and a new room gets created.
		   ...
		   @param sqlLobbyFilter Only used for LobbyType::SQL_LOBBY. This allows 'bigger', 'less', 'or' and 'and' combinations for filtering against certain room properties. Up to 3 ';'-separated filters can be specified. The second filter will
		   only be considered if no rooms could be found that match the first filter and the third filter will only be considered if no rooms could be found that match either the first or the second filter.
		...
		bool Client::opJoinRandomOrCreateRoom(const JString& gameID, const RoomOptions& options, const Hashtable& customRoomProperties, nByte maxPlayers, nByte matchmakingMode, const JString& lobbyName, nByte lobbyType, const JString& sqlLobbyFilter, const JVector<JString>& expectedUsers)
		{
			...
			if((!options.getLobbyName().length() && options.getLobbyType() != LobbyType::DEFAULT) || (!lobbyName.length() && lobbyType != LobbyType::DEFAULT))
			{
				EGLOG(DebugLevel::ERRORS, L"The name of a lobby with a non-default type cannot be an empty string.");
				return false;
			}
			
			OperationRequestParameters op = mpPeer->opJoinRandomRoomImplementation(mRoomName=gameID, RoomOptions(options).setCustomRoomProperties(Hashtable()).setPropsListedInLobby(JVector<JString>()), true, customRoomProperties, maxPlayers, matchmakingMode, lobbyName, lobbyType, sqlLobbyFilter, expectedUsers);
			...
			if(!opCustom(OperationRequest(OperationCode::JOIN_RANDOM_ROOM, op), true))
				return false;
		...
		/**
		   ...
		   @param lobbyType The type of the lobby in which matchmaking should take place. Needs to be one of the values in LobbyType. Note that a lobby with the same name, but a different type
		   can not be considered for matchmaking, as a lobby name only needs to be unique among lobbies of the same type.
		   @param sqlLobbyFilter Only used for LobbyType::SQL_LOBBY. This allows 'bigger', 'less', 'or' and 'and' combinations for filtering against certain room properties. Up to 3 ';'-separated filters can be specified.
		   The second filter will only be considered if no rooms could be found that match the first filter and the third filter will only be considered if no rooms could be found that match either the first or the second filter.
		   @param expectedUsers Sets a list of user IDs for which the server should reserve slots. Those slots can't be taken by other players. This list will be merged with any previously set
		   list of expected users for this room.
		   @returns true, if the request could successfully be queued for sending to the server, false otherwise.
		   @sa
		...
		bool Client::opJoinRandomRoom(const Hashtable& customRoomProperties, nByte maxPlayers, nByte matchmakingMode, const JString& lobbyName, nByte lobbyType, const JString& sqlLobbyFilter, const JVector<JString>& expectedUsers)
		{
			...
			if(!lobbyName.length() && lobbyType != LobbyType::DEFAULT)
			{
				EGLOG(DebugLevel::ERRORS, L"The name of a lobby with a non-default type cannot be an empty string.");
				return false;
			}
			
			if(!mpPeer->opJoinRandomRoom(JString(), RoomOptions(), false, customRoomProperties, maxPlayers, matchmakingMode, lobbyName, lobbyType, sqlLobbyFilter, expectedUsers))
				return false;
        ...
        /**
           ...
           @remarks
           This operation is only available for lobbies of type LobbyType::SQL_LOBBY.

           @param lobby The lobby to query. Has to be of type LobbyType::SQL_LOBBY.
           @param sqlLobbyFilter The sql query statement.
		   @returns true, if the request could successfully be queued for sending to the server, false otherwise.
		   @sa
		   <a href="https://doc.photonengine.com/en-us/realtime/current/reference/matchmaking-and-lobby#sql_lobby_type">sql_lobby_type</a>
		*/
        bool Client::opGetRoomList(const JString& lobby, const JString& sqlLobbyFilter)
		{
			if(getIsOnGameServer())
			{
				EGLOG(DebugLevel::ERRORS, L"room lists can't be retrieved while being inside a game room");
				return false;
			}
            return mpPeer->opGetRoomList(lobby, sqlLobbyFilter);
        }
...
#	pragma GCC diagnostic pop
		...
		/**
		   ...
		   @returns the number of target players, for which the request could successfully be sent (this does not guarantee that it will be received).
		 */
		int Client::sendDirect(const nByte* pParameterArray, int arrSize)
		{
			Common::Serializer s;
			s.push(pParameterArray, arrSize);
			return sendDirect(Common::JVector<nByte>(s.getData(), s.getSize()), JVector<int>(), false);
		}
		...
		/**
		   ...
		   @returns the number of target players, for which the request could successfully be sent (this does not guarantee that it will be received).
		 */
		int Client::sendDirect(nByte* pParameterArray, int arrSize)
		{
			Common::Serializer s;
			s.push(pParameterArray, arrSize);
			return sendDirect(Common::JVector<nByte>(s.getData(), s.getSize()), JVector<int>(), false);
		...
		void Client::onOperationResponse(const OperationResponse& operationResponse)
		{
			...
			case OperationCode::JOIN_ROOM:
				{
					if(getIsOnGameServer())
					{
						...
						case JoinType::JOIN_RANDOM_OR_CREATE_ROOM:
							mListener.joinRandomOrCreateRoomReturn(nr, roomProperties, playerProperties, operationResponse.getReturnCode(), operationResponse.getDebugMessage());
							break;
					...
					else
					{
						...
						case OperationCode::JOIN_ROOM:
							if(operationResponse.getReturnCode())
							{
								...
								{
								case JoinType::JOIN_OR_CREATE_ROOM:
									EGLOG(DebugLevel::ERRORS, L"opJoinOrCreateRoom failed with errorcode %d: %ls. Client is therefore staying on masterserver!", operationResponse.getReturnCode(), operationResponse.getDebugMessage().cstr());
									mListener.joinOrCreateRoomReturn(0, Hashtable(), Hashtable(), operationResponse.getReturnCode(), operationResponse.getDebugMessage());
									break;
								case JoinType::JOIN_ROOM:
			...
			case OperationCode::JOIN_RANDOM_ROOM:
				if(operationResponse.getReturnCode())
				{
					...
					{
					case JoinType::JOIN_RANDOM_OR_CREATE_ROOM:
						EGLOG(DebugLevel::ERRORS, L"opJoinRandomOrCreateRoom failed with errorcode %d: %ls. Client is therefore staying on masterserver!", operationResponse.getReturnCode(), operationResponse.getDebugMessage().cstr());
						mListener.joinRandomOrCreateRoomReturn(0, Hashtable(), Hashtable(), operationResponse.getReturnCode(), operationResponse.getDebugMessage());
						break;
					case JoinType::JOIN_RANDOM_ROOM:
			...
			case OperationCode::GET_REGIONS:
				{
					if(operationResponse.getReturnCode())
					{
						EGLOG(DebugLevel::ERRORS, L"opGetRegions failed with errorcode %d: %ls.", operationResponse.getReturnCode(), operationResponse.getDebugMessage().cstr());
						break;
					}
			...
			case OperationCode::RPC:
				{
					if(operationResponse.getReturnCode())
						EGLOG(DebugLevel::ERRORS, L"opWebRpc failed with errorcode %d: %ls.", operationResponse.getReturnCode(), operationResponse.getDebugMessage().cstr());
					...
					JString* uriPath = uriPathObj.getDataAddress();
				...
				break;
			case OperationCode::GET_ROOM_LIST:
				{
					if(operationResponse.getReturnCode())
					{
						EGLOG(DebugLevel::ERRORS, L"opGetRoomList failed with errorcode %d: %ls.", operationResponse.getReturnCode(), operationResponse.getDebugMessage().cstr());
						break;
					}

					Hashtable rooms = ValueObject<Hashtable>(operationResponse.getParameterForCode(ParameterCode::ROOM_LIST)).getDataCopy();
					JVector<SharedPointer<Room> > roomList;
					JVector<JString> roomNameList;
					const JVector<Object>& keys = rooms.getKeys();
					for(unsigned int i=0; i<keys.getSize(); ++i)
					{
						JString roomNameValue;
						roomNameValue = KeyObject<JString>(keys[i]).getDataCopy();
						roomList.addElement(createRoom(roomNameValue, ValueObject<Hashtable>(rooms.getValue(keys[i])).getDataCopy()));
						roomNameList.addElement(roomNameValue);
					}
					mListener.onGetRoomListResponse(roomList, roomNameList);
				}
				break;
			default:
		...
		void Client::onConnectToMasterFinished(bool comingFromGameserver)
		{
				...
				if(mCachedErrorCodeFromGameServer)
				{
					switch(mLastJoinType)
					{
						case JoinType::CREATE_ROOM:
							...
							break;
						case JoinType::JOIN_OR_CREATE_ROOM:
							mListener.joinOrCreateRoomReturn(0, Hashtable(), Hashtable(), mCachedErrorCodeFromGameServer, mCachedErrorStringFromGameServer);
							break;
						case JoinType::JOIN_RANDOM_OR_CREATE_ROOM:
							mListener.joinRandomOrCreateRoomReturn(0, Hashtable(), Hashtable(), mCachedErrorCodeFromGameServer, mCachedErrorStringFromGameServer);
							break;
						case JoinType::JOIN_ROOM:
			...
			{
				int index = mSelectedRegion.indexOf(L'/');
				mListener.connectReturn(0, L"", index==-1?mSelectedRegion:mSelectedRegion.substring(0, index), mCluster);
			}
		}
		...
		void Client::onArrivalAndAuthentication(void)
		{
			...
			else if(mState == PeerStates::ConnectedToGameserver)
			{
				...
				if(mLastJoinType == JoinType::CREATE_ROOM)
					...
					opJoinOrCreateRoom(mRoomName, RoomOptions(mpCurrentlyJoinedRoom->getIsVisible(), mpCurrentlyJoinedRoom->getIsOpen(), mpCurrentlyJoinedRoom->getMaxPlayers(), mpCurrentlyJoinedRoom->getCustomProperties(), mpCurrentlyJoinedRoom->getPropsListedInLobby(), Common::JString(), LobbyType::DEFAULT, mpCurrentlyJoinedRoom->getPlayerTtl(), mpCurrentlyJoinedRoom->getEmptyRoomTtl(), mpCurrentlyJoinedRoom->getSuppressRoomEvents(), mpCurrentlyJoinedRoom->getPlugins(), mpCurrentlyJoinedRoom->getPublishUserID(), mpCurrentlyJoinedRoom->getDirectMode()), 0, mpCurrentlyJoinedRoom->getExpectedUsers());
