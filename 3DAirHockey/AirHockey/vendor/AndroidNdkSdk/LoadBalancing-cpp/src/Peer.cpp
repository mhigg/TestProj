...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		bool Peer::opJoinLobby(const JString& lobbyName, nByte lobbyType)
		{
			...
			else if(lobbyType != LobbyType::DEFAULT)
			{
		...
		bool Peer::opJoinRandomRoom(const Common::JString& gameID, const RoomOptions& options, bool createIfNotExists, const Hashtable& customRoomProperties, nByte maxPlayers, nByte matchmakingMode, const Common::JString& lobbyName, nByte lobbyType, const Common::JString& sqlLobbyFilter, const JVector<JString>& expectedUsers)
		{
			...
			return opCustom(OperationRequest(OperationCode::JOIN_RANDOM_ROOM, opJoinRandomRoomImplementation(gameID, options, createIfNotExists, customRoomProperties, maxPlayers, matchmakingMode, lobbyName, lobbyType, sqlLobbyFilter, expectedUsers)), true);
		}
		...
		bool Peer::opLobbyStats(const Common::JVector<LoadBalancing::LobbyStatsRequest>& lobbiesToQuery)
		{
			...
			OperationRequestParameters op;
			JVector<JString> names(size);
			JVector<nByte> types(size);
			for(unsigned int i=0; i<size; ++i)
			{
				if(!lobbiesToQuery[i].getName().length() && lobbiesToQuery[i].getType() != LobbyType::DEFAULT)
				{
					EGLOG(DebugLevel::ERRORS, L"The name of a lobby with a non-default type cannot be an empty string.");
					return false;
				}

				names.addElement(lobbiesToQuery[i].getName());
				types.addElement(lobbiesToQuery[i].getType());
			}
			op.put(ParameterCode::LOBBY_NAME, ValueObject<const JString*>(names.getCArray(), size));
			op.put(ParameterCode::LOBBY_TYPE, ValueObject<const nByte*>(types.getCArray(), size));
			bool res = opCustom(OperationRequest(OperationCode::LOBBY_STATS, op), true);
			return res;
		}

		bool Peer::opGetRoomList(const JString& lobby, const JString& sqlLobbyFilter)
		{
			if(!lobby.length())
			{
				EGLOG(DebugLevel::ERRORS, L"'lobby' cannot be an empty string");
				return false;
			}

			if(!sqlLobbyFilter.length())
			{
				EGLOG(DebugLevel::ERRORS, L"'sqlLobbyFilter' cannot be an empty string");
				return false;
			}

			OperationRequestParameters op;
			op.put(ParameterCode::LOBBY_NAME, ValueObject<JString>(lobby));
			op.put(ParameterCode::LOBBY_TYPE, ValueObject<nByte>(LobbyType::SQL_LOBBY));
			op.put(ParameterCode::DATA, ValueObject<JString>(sqlLobbyFilter));

			return opCustom(OperationRequest(OperationCode::GET_ROOM_LIST, op), true);
		}
		...
		OperationRequestParameters Peer::opJoinRandomRoomImplementation(const JString& gameID, const RoomOptions& options, bool createIfNotExists, const Hashtable& customRoomProperties, nByte maxPlayers, nByte matchmakingMode, const JString& lobbyName, nByte lobbyType, const JString& sqlLobbyFilter, const JVector<JString>& expectedUsers)
		{
			OperationRequestParameters op = enterRoomImplementation(createIfNotExists?&options:NULL, Hashtable(), expectedUsers);
			if(gameID.length())
				op.put(ParameterCode::ROOM_NAME, ValueObject<JString>(gameID));
			if(createIfNotExists)
				op.put(ParameterCode::JOIN_MODE, ValueObject<nByte>(JoinMode::CREATE_IF_NOT_EXISTS));
			...
			if(matchmakingMode != MatchmakingMode::FILL_ROOM)
				op.put(ParameterCode::MATCHMAKING_TYPE, ValueObject<nByte>(matchmakingMode));
			if(lobbyName.length()) 
			{
				op.put(ParameterCode::LOBBY_NAME, ValueObject<JString>(lobbyName));
				if(lobbyType != LobbyType::DEFAULT)
		...
		OperationRequestParameters Peer::enterRoomImplementation(const RoomOptions* pOptions, const Hashtable& customLocalPlayerProperties, const JVector<JString>& expectedUsers)
		{
			...
			if(pOptions)
			{
				...
				if(pOptions->getPropsListedInLobby().getSize())
				{
